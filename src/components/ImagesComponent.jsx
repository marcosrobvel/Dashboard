...
const handleSave = () => {console.log("HANDLE SAVE");
  const favourites = !likes;        
  setLikes(favourites);        
  if (favourites) {
    dispatch(toggleFavourite(images));
    const dataWithDescription = {
      ...props,                
      description: ""            
    }            
    icon = "./src/assets/img/heart.svg";            
    ...
    <>
        <div className="image-grid">
            {images.data.map((images, index) => {
                ...
                return <div key={index} className="image-item" onClick={() => openPopup(images.urls.small)}>
                    <div className="divAllImages">
                    <img
                                    src={images.urls.small}
                                    alt="image"
                                    className="cardImage"
                                    onClick={() => openPopup(images.urls.small)}
                                />
                                {isLiked ? (
                                    <svg
                                        ...
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          handleSave(images);
                                          toggleLike(index);  // Evitar que el clic se propague al contenedor
                                        //  handleSave(images);  // Llamamos a handleSave para quitarla de favoritos
                                      }}
                                    >
                                        <path
                                            fillRule="evenodd"
                                    ...
                                    <svg
                                        ...
                                        onClick={(e) => {
                                          ...
                                          handleSave(images);
                                          toggleLike(index); // Evitar que el clic se propague al contenedor
                                          // Llamamos a handleSave para agregarla a favoritos
                                      }}
                                    >
